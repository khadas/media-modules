	...
	unsigned char dv_meta_flag)
{
	...
	if (aux_size > 0 && aux_count > 0) {
		...
		if (new_buf) {
			...
			unsigned char *h = new_buf + pic->aux_data_size;
			unsigned char *p = h + 8;
			int len = 0;
			int padding_len = 0;
			...
			if (get_dbg_flag(hevc) & H265_DEBUG_PRINT_SEI) {
				...
				for (i = 0; i < pic->aux_data_size; i++) {
					hevc_print_cont(hevc, 0, "%02x ", pic->aux_data_buf[i]);
					if (((i + 1) & 0xf) == 0)
						hevc_print_cont(hevc, 0, "\n");
				}
				hevc_print_cont(hevc, 0, "\n");
...
static void hevc_local_uninit(struct hevc_state_s *hevc)
{
	...
	if (hevc->rpm_addr) {
		dma_free_coherent(amports_get_dma_device(),
				RPM_BUF_SIZE, hevc->rpm_addr, hevc->rpm_phy_addr);
		hevc->rpm_addr = NULL;
	}
	if (hevc->lmem_addr) {
		dma_free_coherent(amports_get_dma_device(),
				RPM_BUF_SIZE, hevc->lmem_addr, hevc->lmem_phy_addr);
		hevc->lmem_addr = NULL;
	}
	...
	if (state == RECEIVER_INACTIVE && hevc->timeout_flag &&
	    ...
	    if (new_pic)  {
			...
			for (i = 0; i < MAX_REF_PIC_NUM; i++) {
				...
				if ((pic->referenced == 1) && (pic->error_mark == 1)) {
					flush_output(hevc, pic);
					hevc_print(hevc, 0, "check_buffer_status DPB error, neeed force flush  receive_state %d \n", state);
					break;
				}
			}
		}
