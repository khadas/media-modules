...
#if 0
#define PRINT_FUNC_LINE() pr_info("%s %d\n", __func__, __LINE__)
#define DEBUG_PRINT_FUNC_LINE(str)  pr_info("%s, %d, %s\n", __func__, __LINE__, str)
#else
#define PRINT_FUNC_LINE() if (0) {;}
#define DEBUG_PRINT_FUNC_LINE(str) if (0) {;}
#endif
...
static void timeout_process_back(struct AV1HW_s *hw)
{
	...
	if (work_pending(&hw->work_back) ||
		work_busy(&hw->work_back) ||
		work_busy(&hw->timeout_work_back) ||
		work_pending(&hw->timeout_work_back)) {
		av1_print(hw, 0, "%s AV1[%d] work back pending, do nothing.\n",__func__, hw->index);
		return;
	}
...
static int av1_mmu_page_num(struct AV1HW_s *hw,
		int w, int h, int is_bitdepth_10)
{
	int picture_size;
	int cur_mmu_4k_number, max_frame_num;

	picture_size = compute_losless_comp_body_size(w, h, is_bitdepth_10);
...
int av1_alloc_mmu_dw(
	struct AV1HW_s *hw, void *mmu_box,
	int cur_buf_idx,
	int pic_width,
	int pic_height,
	unsigned short bit_depth,
...
static struct BuffInfo_s aom_workbuff_spec[WORK_BUF_SPEC_NUM] = {
	{ //8M bytes
		...
		.seg_map        = {.buf_size = 0xCA800,}, // SEGMENT MAP AREA - 1920x1088/4/4 * 3 bits = 0xBF40 Bytes * 16 = 0xBF400
		.daala_top      = {.buf_size = 0xf00,}, // DAALA TOP STORE AREA - 224 Bytes (use 256 Bytes for LPDDR4) per 128. Total 4096/128*256 = 0x2000
		...
		.dblk_para      = {.buf_size = 0xd00,}, /*0xc40*/// DBLK -> Max 256(4096/16) LCU, each para 1024bytes(total:0x40000), data 1024bytes(total:0x40000)
		.dblk_data      = {.buf_size = 0x49000,},
		.cdef_data      = {.buf_size = 0x22400,},
		.ups_data       = {.buf_size = 0x36000,},
...
#endif
		.mpred_above    = {.buf_size = 0x2800, },/*round from 0x2760*/ /* 2 * size of hw*/
#ifdef MV_USE_FIXED_BUF
	...
	},
	{
		...
		.short_term_rps = {.buf_size = 0x800,}, // SHORT_TERM_RPS - Max 64 set, 16 entry every set, total 64x16x2 = 2048 bytes (0x800)
		.vps            = {.buf_size = 0x800,}, // VPS STORE AREA - Max 16 VPS, each has 0x80 bytes, total 0x0800 bytes
		.seg_map        = {.buf_size = 0x360000,}, // SEGMENT MAP AREA - 4096x2304/4/4 * 3 bits = 0x36000 Bytes * 16 = 0x360000
...
#endif
		.mpred_above    = {.buf_size = 0x5400,}, /* 2 * size of hw*/
#ifdef MV_USE_FIXED_BUF
	...
	},
	{
		...
		.short_term_rps = {.buf_size = 0x800,}, // SHORT_TERM_RPS - Max 64 set, 16 entry every set, total 64x16x2 = 2048 bytes (0x800)
		.vps            = {.buf_size = 0x800,}, // VPS STORE AREA - Max 16 VPS, each has 0x80 bytes, total 0x0800 bytes
		.seg_map        = {.buf_size = 0xd80000,}, // SEGMENT MAP AREA - 4096x2304/4/4 * 3 bits = 0x36000 Bytes * 16 = 0x360000
		.daala_top      = {.buf_size = 0x2000,}, // DAALA TOP STORE AREA - 224 Bytes (use 256 Bytes for LPDDR4) per 128. Total 4096/128*256 = 0x2000
		.sao_up         = {.buf_size = 0x0,}, // //0x2800, SAO UP STORE AREA - Max 640(10240/16) LCU, each has 16 bytes total 0x2800 bytes
		.swap_buf       = {.buf_size = 0x800,}, // 256cyclex64bit = 2K bytes 0x800 (only 144 cycles valid)
		.cdf_buf        = {.buf_size = 0x80000,}, // for context store/load 1024x256 x16 = 512K bytes 16*0x8000
		.gmc_buf        = {.buf_size = 0x800,}, 	// for gmc_parameter store/load 128 x 16 = 2K bytes 0x800
		.scalelut       = {.buf_size = 0x0, }, ////0x8000, support up to 32 SCALELUT 1024x32 = 32Kbytes (0x8000)
		.dblk_para      = {.buf_size = 0x3300, }, // /*0x32a0*/ DBLK -> Max 256(4096/16) LCU, each para 1024bytes(total:0x40000), data 1024bytes(total:0x40000)
		.dblk_data      = {.buf_size = 0xa4800,},
		.cdef_data      = {.buf_size = 0x29200,},
		.ups_data       = {.buf_size = 0xdb000,},
		.fgs_table      = {.buf_size = FGS_TABLE_SIZE * FRAME_BUFFERS,}, // 512x128bits
#ifdef AOM_AV1_MMU
...
#endif
		.mpred_above    = {.buf_size = 0xA800,}, /* 2 * size of hw*/
#ifdef MV_USE_FIXED_BUF
	...
	{ //8M bytes
		...
		.dblk_para      = {.buf_size = 0x80000,}, // DBLK -> Max 256(4096/16) LCU, each para 1024bytes(total:0x40000), data 1024bytes(total:0x40000)
		.dblk_data      = {.buf_size = 0xa4800,}, //0x80000,},
		.cdef_data      = {.buf_size = 0x30000,},	// cdef_temp_address start
		.ups_data       = {.buf_size = 0x130000,},	// ups_temp_address start
...
#endif
		.mpred_above    = {.buf_size = 0x10000,}, /* 2 * size of hevc*/
#ifdef MV_USE_FIXED_BUF
...
#ifdef AOM_AV1_MMU_DW
	if (hw->mmu_box_dw_1)
		decoder_mmu_box_free(hw->mmu_box_dw_1);
	hw->mmu_box_dw_1 = NULL;
#endif
#endif
...
//#endif
	...
	if (hw->mmu_enable) {
		struct internal_comp_buf *ibuf = v4lfb_to_icomp_buf(hw, aml_buf);
...
static void dump_pic_list(struct AV1HW_s *hw)
{
	...
	for (i = 0; i < FRAME_BUFFERS; i++) {
		pic_config = &cm->buffer_pool->frame_bufs[i].buf;
		av1_print(hw, 0,
			"Buf(%d) index %d mv_buf_index %d ref_count %d vf_ref %d dec_idx %d slice_type %d w/h %d/%d adr%ld back_done %d, backendref %d\n",
			i,
			pic_config->index,
#ifndef MV_USE_FIXED_BUF
	...
	if (vf) {
		...
		if (without_display_mode == 0) {
			if (hw->is_used_v4l) {
				...
				if (hw->front_back_mode == 0 || hw->front_back_mode == 2)
...
#ifdef NEW_FB_CODE
		if (ret >= 0 && (hw->front_back_mode != 1) && hw->mmu_enable && ((hw->double_write_mode & 0x10) == 0)) {
#else
...
static irqreturn_t vav1_isr_thread_fn(int irq, void *data)
{
	...
	DEBUG_PRINT_FUNC_LINE("front thread irq start");
	...
	if ((dec_status == AOM_NAL_DECODE_DONE) ||
		...
		if (hw->front_back_mode != 1) {
			...
			DEBUG_PRINT_FUNC_LINE("pic end in");
		...
		if (hw->m_ins_flag) {
			...
			READ_VREG(HEVC_SHIFT_BYTE_COUNT) < hw->data_size) {
				if (enable_single_slice == 1) {
					...
					if (hw->front_back_mode == 1) {
						amhevc_stop_f();
						//save_stream_context(pbi->work_space_buf->swap_buf.buf_start);
					} else
#endif
	...
	DEBUG_PRINT_FUNC_LINE("load_param in");
...
#if 0
//#ifdef NEW_FB_CODE
	if (hw->stat & STAT_VDEC_BACK_RUN)
		amhevc_stop_b();
		hw->stat &= ~STAT_VDEC_BACK_RUN;
	}
#endif
...
static int amvdec_av1_mmu_init(struct AV1HW_s *hw)
{
	...
	if (hw->mmu_enable && !hw->is_used_v4l) {
		...
		if (hw->front_back_mode) {
			...
			if (!hw->mmu_box_1) {
				pr_err("av1 alloc mmu box 1 failed!!\n");
				return -1;
			}
		}
#endif
		...
		if (hw->dw_mmu_enable) {
			...
			if (hw->front_back_mode) {
				...
				if (!hw->mmu_box_dw_1) {
					pr_err("av1 alloc dw mmu box 1 failed!!\n");
					return -1;
				}
			}
#endif
...
static unsigned long run_ready_back(struct vdec_s *vdec, unsigned long mask)
{
	...
	if (hw->timeout_processing_back &&
		(work_pending(&hw->work_back) ||
		work_busy(&hw->work_back) ||
		work_busy(&hw->timeout_work_back) ||
		work_pending(&hw->timeout_work_back))) {
		av1_print(hw, 0,
			"av1 work back pending,not ready for run.\n");
		return 0;
	}
...
void pic_compress_header_dump(struct AV1HW_s *hw, PIC_BUFFER_CONFIG *cur_pic, u32 pic_id)
{
	...
	fp = file_open(O_CREAT | O_RDWR | O_TRUNC, "/data/tmp/test0081_header%d.bin", pic_id);
	if (fp == NULL)
		return;
		...
		(debug & AOM_DEBUG_DIS_RECYCLE_MMU_TAIL) == 0) {
			...
			if (pic != NULL) {
				...
				*/
				...
				decoder_mmu_box_free_idx_tail(hw->mmu_box,
					pic->index, used_4k_num0);

				decoder_mmu_box_free_idx_tail(hw->mmu_box_1,
				...
				if (hw->dw_mmu_enable) {
					...
					decoder_mmu_box_free_idx_tail(hw->mmu_box_dw_1,
						pic->index, used_4k_num1);
					av1_print(hw, AOM_DEBUG_HW_MORE,
...
static int ammvdec_av1_probe(struct platform_device *pdev)
{
	...
	if (pdata == NULL) {
		pr_err("ammvdec_av1 memory resource undefined. pdata null\n");
		return -EFAULT;
	}
...
multi_probe_failed3:
	av1_local_uninit(hw);	//free alloced dma memory, gvs
multi_probe_failed2:
	uninit_mmu_buffers(hw);
multi_probe_failed1:
	vfree(hw->pbi);
...
module_param(disable_force_dw, uint, 0664);
MODULE_PARM_DESC(disable_force_dw, "\n disable_force_dw\n");
