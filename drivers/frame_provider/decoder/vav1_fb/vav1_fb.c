		...
		if (buf_size > 0) {
			...
			if (vdec->vdata != NULL) {
				...
				if (index >= 0) {
					...
					decoder_bmmu_box_add_callback_func(hw->bmmu_box, VF_BUFFER_IDX(i),
						(void *)&vdec->vdata->release_callback[j]);
				} else {
					av1_print(hw, 0, "vdec data is full\n");
				}
			}
...
static int __init amvdec_av1_driver_init_module(void)
{
	...
	register_set_debug_flag_func(DEBUG_AMVDEC_AV1_FB, set_debug_flag);
#ifdef ERROR_HANDLE_DEBUG
	dbg_nal_skip_flag = 0;
	...
	vcodec_profile_register_v2("av1_fb", VFORMAT_AV1, 0);
