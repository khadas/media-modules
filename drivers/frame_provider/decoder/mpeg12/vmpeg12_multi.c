...
static irqreturn_t vmpeg12_isr_thread_handler(struct vdec_s *vdec, int irq)
{
	...
	if (reg == MPEG12_DATA_REQUEST) {
		...
		debug_print(DECODE_ID(hw), PRINT_FLAG_RUN_FLOW,
			"mmpeg12: disp_pic=%d(%c), ind=%d, offst=%x, poc %d, pts=(%d,%lld,%llx)(%d)\n",
...
static void vmpeg2_dump_state(struct vdec_s *vdec)
{
	...
	debug_print(DECODE_ID(hw), 0,
		"====== %s\n", __func__);
	debug_print(DECODE_ID(hw), 0,
		"width/height (%d/%d),i_first %d, buf_num %d, run_flag %d, error_frame_skip_level %d\n",
		hw->frame_width,
		hw->frame_height,
...
static int __init ammvdec_mpeg12_driver_init_module(void)
{
	...
	vcodec_profile_register_v2("mmpeg12", VFORMAT_MPEG12, 0);
	...
	register_set_debug_flag_func(DEBUG_AMVDEC_MPEG12, set_debug_flag);
