...
#define MULTI_DRIVER_NAME "ammvdec_avs2_fb_v4l"
...
static int avs2_recycle_frame_buffer(struct AVS2Decoder_s *dec);
...
{
	/*
	 * In this very timeout point,the avs2_work arrives,
	...
	avs2_print(dec, 0, "%s decoder timeout, lcu 0x%x \n",
...
static int get_idle_pos(struct AVS2Decoder_s *dec)
{
	struct avs2_decoder *avs2_dec = &dec->avs2_dec;
...
static int v4l_get_free_fb(struct AVS2Decoder_s *dec)
{
	...
	if (debug & AVS2_DBG_OUT_PTS) {
		if (free_pic) {
			pr_debug("%s, ref:%d, buf:%d, ts:%lld\n",
				__func__, free_ref_idx,
				free_pic->index,
				free_pic->timestamp);
		} else {
...
static struct BuffInfo_s amvavs2_workbuff_spec[WORK_BUF_SPEC_NUM] = {
	...
	},
	{
		...
		.cm_header = {
			.buf_size = MMU_COMPRESS_HEADER_SIZE * AVS2_BUFFER_NUM,
		},
#endif
		...
		.mmu_vbh_dw = {
			...
			.cm_header_dw = {
			.buf_size = MMU_COMPRESS_HEADER_SIZE_DW * AVS2_BUFFER_NUM,
		},
#endif
#else
#ifdef AVS2_10B_MMU
...
static int front_decpic_done_update(struct AVS2Decoder_s *dec, uint8_t reset_flag)
{
	...
	if (dec->front_back_mode) {
		dec->avs2_dec.hc.cur_pic->backend_ref = 1;
		for (i = 0; i < MAXREF; i++) {
			if (dec->avs2_dec.hc.cur_pic->ref_pic[i])
				dec->avs2_dec.hc.cur_pic->ref_pic[i]->backend_ref++;
		}
	...
	if (dec->front_back_mode == 1) {
		...
		WRITE_VREG(HEVC_DEC_STATUS_REG, AVS2_DEC_IDLE);
...
{
	memcpy(vf, vavs2_vf_get(vdec_ctx), sizeof(struct vframe_s));
}
...
static void dump_pic_list(struct AVS2Decoder_s *dec)
{
	...
	for (ii = 0; ii < avs2_dec->ref_maxbuffer; ii++) {
		avs2_print(dec, 0,
			"fref[%d]: index %d decode_id %d mvbuf %d imgcoi_ref %d imgtr_fwRefDistance %d refered %d, pre %d is_out %d, bg %d, vf_ref %d error %d lcu %d ref_pos(%d,%d,%d,%d,%d,%d,%d) backend_ref %d\n",
...
static void config_sao_hw(struct AVS2Decoder_s *dec)
{
	...
	if (get_cpu_major_id() == AM_MESON_CPU_MAJOR_ID_T3X) {
		if (tw_mode) {
			...
			WRITE_VREG(HEVC_SAO_Y_LENGTH3, cur_pic->luma_size_ex);
			WRITE_VREG(HEVC_SAO_C_LENGTH3, cur_pic->chroma_size_ex);
			...
			avs2_print(dec, PRINT_FLAG_V4L_DETAIL, "[%d] config tw, id: %d, Y:(%x, %d) C:(%x, %d).\n",
				v4l2_ctx->id, cur_pic->index,
				cur_pic->tw_y_adr, cur_pic->luma_size_ex,
				cur_pic->tw_u_v_adr, cur_pic->chroma_size_ex);
		} else {
	...
	*/
	...
	if (dw_mode & 0x10) {
		...
		*/
		...
		/*set them all 0 for AVS2_NV21 (no down-scale)*/
		data32 &= ~(0xff << 16);
...
static void v4l_submit_vframe(struct AVS2Decoder_s *dec)
{
...
static void avs2_recycle_mmu_buf_tail(struct AVS2Decoder_s *dec)
{
	...
	if (dec->cur_fb_idx_mmu != INVALID_IDX) {
		if (dec->used_4k_num == -1) {
			...
			ctx->cal_compress_buff_info(dec->used_4k_num, ctx);
...
static int avs2_mmu_page_num(struct AVS2Decoder_s *dec,
		int w, int h, int is_bit_depth_10)
{
	int picture_size;
...
decode_slice:
			...
			if (dec->front_back_mode == 1 ||
				...
				cur_imem_ptr_v = avs2_dec->fr.sys_imem_ptr_v;
				avs2_print(dec, PRINT_FLAG_VDEC_DETAIL,
				...
				if (sys_imem_ptr >= avs2_dec->fb_buf_sys_imem.buf_end) {
					...
					sys_imem_ptr = avs2_dec->fb_buf_sys_imem.buf_start;
					avs2_dec->sys_imem_ptr_v = dec->fb_buf_sys_imem_addr;
				 }
				if (dec->front_back_mode == 1) {
				...
				memcpy(cur_imem_ptr_v, (void*)(&avs2_dec->instruction[0]), avs2_dec->ins_offset*4);
			}
			else
#endif
			{
...
static bool is_available_buffer(struct AVS2Decoder_s *dec)
{
	...
	if ((dec->used_buf_num == 0) ||
		...
		avs2_dec->ref_maxbuffer = dec->used_buf_num - 1;
	}
...
#ifdef NEW_FB_CODE
		if (dec->front_back_mode) {
			mmu_box_1 = ctx->bm.mmu_1;
			size_1 = decoder_mmu_box_sc_check(mmu_box_1, tvp);
		}
#endif
...
static void run_back(struct vdec_s *vdec, void (*callback)(struct vdec_s *, void *, int), void *arg)
{
	...
	if (vdec->mc_back_loaded || dec->front_back_mode != 1) {
		...
		*/
		...
		vdec->mc_back_type = VFORMAT_AVS2;
	}
...
static int ammvdec_avs2_probe(struct platform_device *pdev)
{
	...
	ctx->vdec_recycle_dec_resource = avs2_recycle_dec_resource;
	pdata->private = dec;
