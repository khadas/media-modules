...
#define DRIVER_NAME "amvdec_avs3_v4l"
...
static void timeout_process_back(struct AVS3Decoder_s *dec)
{
	...
	avs3_print(dec, 0, "%s decoder timeout\n", __func__);
...
#if 0
static struct aml_buf *index_to_afbc_aml_buf(struct AVS3Decoder_s *dec, int index)
{
	return (struct aml_buf *)dec->afbc_buf_table[index].fb;
}
#endif

static struct aml_buf *index_to_aml_buf(struct AVS3Decoder_s *dec, int index)
{
	return (struct aml_buf *)dec->m_BUF[index].v4l_ref_buf_addr;
}

#if 0
static int avs3_get_current_fbc_index(struct AVS3Decoder_s *dec, int index)
{
	int i;

	for (i = 0; i < BUF_FBC_NUM_MAX; i++) {
		if (dec->m_BUF[index].v4l_ref_buf_addr
			== dec->afbc_buf_table[i].fb) {
			avs3_print(dec, AVS3_DBG_BUFMGR, "cur fb idx mmu %d\n", i);
			break;
		}
	}

	if (i >= BUF_FBC_NUM_MAX)
		avs3_print(dec, 0, "[ERR]can't find fb(0x%lx) in afbc table\n",
	...
	unsigned int *mmu_index_adr)
{
	...
	avs3_print(dec, AVS3_DBG_BUFMGR_MORE,
		...
		__func__, cur_buf_idx, aml_buf->fbc->frame_size);
...
static int check_free_buf_count(struct AVS3Decoder_s *dec)
{
	struct avs3_decoder *avs3_dec = &dec->avs3_dec;
	int i;
	int free_slot = 0;
	...
	for (i = 0; i < avs3_dec->max_pb_size; ++i) {
		if ((avs3_dec->pic_pool[i].buf_cfg.index != -1) &&
			(avs3_dec->pic_pool[i].buf_cfg.bg_flag == 0) &&
#ifdef NEW_FRONT_BACK_CODE
			(avs3_dec->pic_pool[i].buf_cfg.backend_ref == 0) &&
#endif
			(avs3_dec->pic_pool[i].buf_cfg.vf_ref == 0) &&
			(avs3_dec->pic_pool[i].buf_cfg.used == 0) &&
			(!avs3_dec->pic_pool[i].buf_cfg.cma_alloc_addr)) {
			free_slot++;
		}
	}
...
static int get_free_buf_count(struct AVS3Decoder_s *dec)
{
	...
	free_slot = check_free_buf_count(dec);
	...
	if (!free_slot) {
		...
		for (i = 0; i < avs3_dec->max_pb_size; ++i) {
			avs3_print(dec,
				PRINT_FLAG_VDEC_DETAIL, "%s idx %d bg_flag %d vf_ref %d "
				"used %d index %d backend_ref %d cma_alloc_addr = 0x%lx\n",
				__func__, i,
				avs3_dec->pic_pool[i].buf_cfg.bg_flag,
				avs3_dec->pic_pool[i].buf_cfg.vf_ref,
				avs3_dec->pic_pool[i].buf_cfg.used,
				avs3_dec->pic_pool[i].buf_cfg.index,
				avs3_dec->pic_pool[i].buf_cfg.backend_ref,
				avs3_dec->pic_pool[i].buf_cfg.cma_alloc_addr);
		}
...
static int get_idle_pos(struct AVS3Decoder_s *dec)
{
	struct avs3_decoder *avs3_dec = &dec->avs3_dec;
	int i;
	for (i = 0; i < avs3_dec->max_pb_size; ++i) {
...
static int v4l_get_free_fb(struct AVS3Decoder_s *dec)
{
	...
	pic = &avs3_dec->pic_pool[pos].buf_cfg;
	...
	if (free_pic) {
		avs3_print(dec, AVS3_DBG_OUT_PTS, "%s, idx: %d, ts: %lld, free_ref_idx %d\n",
			__func__, free_pic->index, free_pic->timestamp, free_ref_idx);
	} else {
...
static uint32_t get_mv_buf_size(struct AVS3Decoder_s *dec, int width, int height) {
	...
	if (mv_buf_dynamic_alloc == 1) {
		int mv_mem_unit = get_mv_mem_unit(avs3_dec->lcu_size_log2);
...
static void dealloc_mv_bufs(struct AVS3Decoder_s *dec)
{
	int i;
	for (i = 0; i < FRAME_BUFFERS; i++) {
		...
		dec->avs3_dec.pic_pool[i].buf_cfg.mv_buf_index = -1;
	}
}

static int alloc_mv_buf(struct AVS3Decoder_s *dec, int i)
{
	int ret = 0;
...
static void put_mv_buf(struct AVS3Decoder_s *dec, struct avs3_frame_s *pic)
{
	...
	avs3_print(dec, AVS3_DBG_BUFMGR_MORE,
		"%s(%d): used_flag(%d)\n",
		__func__, i, dec->m_mv_BUF[i].used_flag);
...
static int get_mv_buf(struct AVS3Decoder_s *dec, struct avs3_frame_s *pic)
{
	int i;
	int ret = -1;
	int new_size;

	new_size = get_mv_buf_size(dec, pic->width, pic->height);
	...
	if (pic->mv_buf_index != -1) {
		avs3_print(dec, AVS3_DBG_BUFMGR, "%s : warning, mv buff:%d hasn't been released\n",
	...
	if (ret >= 0) {
		...
		avs3_print(dec, 0, "%s: Error, mv buf is not enough\n", __func__);
	}
	return ret;
}
#endif

static void put_un_used_mv_bufs(struct AVS3Decoder_s *dec)
{
#ifndef MV_USE_FIXED_BUF
	struct avs3_decoder *avs3_dec = &dec->avs3_dec;
	int ii;
	for (ii = 0; ii < avs3_dec->max_pb_size; ii++) {
		if (avs3_dec->pic_pool[ii].is_ref == 0 &&
			avs3_dec->pic_pool[ii].buf_cfg.backend_ref == 0 &&
			avs3_dec->pic_pool[ii].buf_cfg.mv_buf_index >= 0)
			put_mv_buf(dec, &avs3_dec->pic_pool[ii].buf_cfg);
	}
#endif
}
...
int find_near_pic_index(struct AVS3Decoder_s *dec)
{
	struct avs3_decoder *avs3_dec = &dec->avs3_dec;
	...
	for (i = 0; i < avs3_dec->max_pb_size; i++) {
		pic = &avs3_dec->pic_pool[i];
		if ((pic == NULL) || (pic->buf_cfg.used == 0))
			continue;

		if (abs(cur_pic->poc - pic->ptr) < diff) {
			diff = abs(cur_pic->poc - pic->ptr);
			index = i;
		}
	}
...
static int check_pic_decoded_lcu(struct AVS3Decoder_s *dec, struct avs3_frame_s *pic)
{
...
static int front_decpic_done_update(struct AVS3Decoder_s *dec, uint8_t reset_flag)
{
	...
	if ((!(error_handle_policy & 0x4))
		...
		if (cur_pic->decoded_lcu != dec->avs3_dec.lcu_total) {
			...
			if (index != -1)
				pic = &avs3_dec->pic_pool[index].buf_cfg;

			if (pic != NULL) {
				cur_pic->need_mmu_copy = 1;
				cur_pic->copy_pic = pic;
				mutex_lock(&dec->fb_mutex);
				pic->backend_ref++;
				mutex_unlock(&dec->fb_mutex);
...
{
	memcpy(vf, vavs3_vf_get(vdec_ctx), sizeof(struct vframe_s));
}
	...
	struct avs3_frame_s *pic)
{
...
#ifdef AVS3_10B_MMU
	if (dec->mmu_enable) {
		dec->m_BUF[i].header_addr = aml_buf->fbc->haddr;
	...
	if (aml_buf->num_planes == 1) {
		if (dw_mode) {
			dec->m_BUF[i].start_adr = aml_buf->planes[0].addr;
			dec->m_BUF[i].luma_size = aml_buf->planes[0].offset;
			dec->m_BUF[i].size = aml_buf->planes[0].length;
		...
		if (dw_mode) {
			dec->m_BUF[i].start_adr = aml_buf->planes[0].addr;
			dec->m_BUF[i].size = aml_buf->planes[0].length;
			dec->m_BUF[i].chroma_addr = aml_buf->planes[1].addr;
			dec->m_BUF[i].chroma_size = aml_buf->planes[1].length;
			aml_buf->planes[0].bytes_used = aml_buf->planes[0].length;
...
static int config_mc_buffer(struct AVS3Decoder_s *dec)
{
	...
	//    return 0;
	avs3_print(dec, AVS3_DBG_BUFMGR_MORE, "Entered config_mc_buffer....\n");
	...
	if (avs3_dec->slice_type == SLICE_B) {
		avs3_print(dec, AVS3_DBG_BUFMGR_DETAIL, "config_mc_buffer for REF_1\n");
...
static void config_mpred_hw(struct AVS3Decoder_s *dec)
{
	...
	data32 = (avs3_dec->ctx.dpm.num_refp[REFP_0] |
...
static void config_dblk_hw(struct AVS3Decoder_s *dec)
{
	...
	*/
	...
	avs3_print(dec, AVS3_DBG_BUFMGR_DETAIL, "[c] cfgDBLK: .done.\n");
}
...
static void avs3_recycle_dec_resource(void *priv,
	struct aml_buf *aml_buf)
{
}
...
static void vavs3_vf_put(struct vframe_s *vf, void *op_arg)
{
	...
	avs3_recycle_dec_resource(dec, aml_buf);
...
static void avs3_recycle_mmu_buf_tail(struct AVS3Decoder_s *dec)
{
	if (dec->cur_fb_idx_mmu != INVALID_IDX) {
		...
		decoder_mmu_box_free_idx_tail(aml_buf->fbc->mmu, aml_buf->fbc->index, used_4k_num);
...
irqreturn_t vavs3_back_isr_thread_fn(struct AVS3Decoder_s *dec)
{
	...
	if (debug & AVS3_DBG_IRQ_EVENT)
		avs3_print(dec, 0, "[BE] avs3 back isr dec_status_back  = 0x%x\n", dec->dec_status_back);
	if (READ_VREG(DEBUG_REG1_DBE)) {
		...
		if ((dec->front_back_mode == 1 ||
			...
			&& (pic->need_mmu_copy == 0)) {
				...
				avs3_print(dec, AVS3_DBG_BUFMGR_MORE,
					...
					__func__, pic->index, aml_buf->fbc->index,
...
static void avs3_buf_ref_process_for_exception(struct AVS3Decoder_s *dec, bool is_front)
{
	...
	if ((is_front) && (dec->cur_idx != INVALID_IDX)) {
		int cur_idx = dec->cur_idx;
		aml_buf = (struct aml_buf *)dec->m_BUF[cur_idx].v4l_ref_buf_addr;

		avs3_print(dec, 0,
			"process_for_exception: dma addr(0x%lx)\n",
			avs3_dec->pic_pool[cur_idx].buf_cfg.cma_alloc_addr);

		aml_buf_put_ref(&ctx->bm, aml_buf);
		aml_buf_put_ref(&ctx->bm, aml_buf);

		avs3_dec->pic_pool[cur_idx].buf_cfg.cma_alloc_addr = 0;
		avs3_dec->pic_pool[cur_idx].buf_cfg.vf_ref = 0;
		dec->m_BUF[cur_idx].v4l_ref_buf_addr = 0;

		dec->cur_idx = INVALID_IDX;
	} else 	if ((is_front == false) && (dec->cur_back_idx != INVALID_IDX)) {
		int cur_idx = dec->cur_back_idx;
		aml_buf = (struct aml_buf *)dec->m_BUF[cur_idx].v4l_ref_buf_addr;
...
static irqreturn_t vavs3_isr_thread_fn(int irq, void *data)
{
				...
				if (pic->used == 0 &&
					...
					if (dec->dw_mmu_enable && aml_buf->fbc->mmu_dw) {
						...
						if (dec->front_back_mode && aml_buf->fbc->mmu_dw_1)
							decoder_mmu_box_free_idx(aml_buf->fbc->mmu_dw_1, pic->index);
					}
#endif
				}
			}
	...
	if ((dec_status == AVS3_HEAD_PIC_I_READY)
		...
		if (dec->avs3_dec.param.p.video_signal_type & (1<<14)) {
			...
			dec->vf_dp.content_light_level.max_pic_average = pPara->p.max_picture_average_light_level;
	...
	if (start_code == SEQUENCE_HEADER_CODE) {
		avs3_print(dec, AVS3_DBG_BUFMGR_MORE, " ## SEQUENCE_HEADER_CODE ##\n");
	...
	if (start_code == SEQUENCE_END_CODE) {
		...
		if (ret == 0) {
			...
			if (dec->mmu_enable
				...
				//dec->cur_fb_idx_mmu = avs3_get_current_fbc_index(dec, dec->avs3_dec.cur_pic->index);
				...
				if (ret >= 0) {
					...
					avs3_print(dec, 0,"can't alloc need mmu1,idx %d ret =%d\n",
						cur_pic->index, ret);
			}
#endif
#ifdef AVS3_10B_MMU_DW
			if (dec->dw_mmu_enable
				...
				if (ret >= 0) {
					...
					avs3_print(dec, 0,"can't alloc need dw mmu1,idx %d ret =%d\n",
						cur_pic->index, ret);
			}
#endif
		}
...
decode_slice:
	...
	if (efficiency_mode && (dec->front_back_mode == 1)) {
		if (dec->pic_list_init_flag && (first_init_pic_list == 0)) {
			init_pic_list_hw_fb(dec);
		}
		if ((dec->front_back_mode == 1 || dec->front_back_mode == 3) &&
			(start_code == I_PICTURE_START_CODE ||
...
static void avs3_check_timer_back_func(struct timer_list *timer)
{
	...
	if (fast_timer_check_count != 0) {
		...
		if (dec->decode_timeout_count_back == 0) {
			...
			if (dec->last_monitor_data == tmp_ddr_data)
				...
				dec->decode_timeout_count_back = 1;
	...
	} else if ((fast_timer_check_count != 0) &&
		(fast_timer_check_count > 0) &&
		(dec->start_process_time_back > 0) &&
...
static void vavs3_put_timer_func(struct timer_list *timer)
{
			...
			if (dec->last_lcu_idx == current_lcu_idx) {
				...
				if (dec->decode_timeout_count == 0) {
					if (input_frame_based(hw_to_vdec(dec)) ||
						...
						avs3_print(dec, 0, "timeout & empty, again\n");
						dec_again_process(dec);
					}
				}
			} else {
	...
	if (pop_shorts != 0) {
		...
		for (i = 0; i < pop_shorts; i++) {
			u32 data = (READ_HREG(HEVC_SHIFTED_DATA) >> 16);
			WRITE_VREG(HEVC_SHIFT_COMMAND, (1<<7)|16);
			if ((i & 0xf) == 0)
				pr_info("%04x:", i);
			pr_info("%04x ", data);
			if (((i + 1) & 0xf) == 0)
...
static int vavs3_local_init(struct AVS3Decoder_s *dec)
{
	...
	if (NULL == dec->gvs) {
		avs3_print(dec, 0, "the struct of vdec status malloc failed.\n");
		return -1;
	}
#ifdef DEBUG_PTS
	dec->pts_missed = 0;
...
static int amvdec_avs3_probe(struct platform_device *pdev)
{
	...
	if (get_cpu_major_id() >= AM_MESON_CPU_MAJOR_ID_T5D) {
		dec->dw_mmu_enable = (get_double_write_mode(dec) & 0x20) ? 1 : 0;
	} else {
		dec->dw_mmu_enable = 0;
	}
#endif
	...
	hevc_source_changed(VFORMAT_AVS3, 4096, 2048, 60);
	mutex_unlock(&vavs3_mutex);
...
static void dump_data(struct AVS3Decoder_s *dec, int size)
{
	int jj;
	u8 *data = NULL;
	int padding_size = dec->chunk->offset & (VDEC_FIFO_ALIGN - 1);
	...
	for (jj = padding_size; jj > 0; jj--)
		avs3_print_cont(dec, 0, "%02x ", *(data - jj));
	avs3_print_cont(dec, 0, "data adr %p\n", data);

	for (jj = 0; jj < size; jj++) {
		if ((jj & 0xf) == 0)
			avs3_print(dec, 0, "%06x:", jj);
		avs3_print_cont(dec, 0, "%02x ", data[jj]);
		if (((jj + 1) & 0xf) == 0)
			avs3_print(dec, 0, "\n");
	}
...
static void avs3_work_implement(struct AVS3Decoder_s *dec)
{
	...
	if (((dec->dec_result == DEC_RESULT_GET_DATA) ||
		...
		if (get_free_buf_count(dec) >= dec->run_ready_min_buf_num) {
			...
			decode_size = dec->chunk->size + (dec->chunk->offset & (VDEC_FIFO_ALIGN - 1));

			WRITE_VREG(HEVC_DECODE_SIZE, READ_VREG(HEVC_DECODE_SIZE) + decode_size);
			...
			avs3_print(dec, PRINT_FLAG_VDEC_DETAIL, "amvdec_avs3: Insufficient data\n");
...
static void avs3_work_back(struct work_struct *work)
{
	struct AVS3Decoder_s *dec = container_of(work, struct AVS3Decoder_s, work_back);
	struct vdec_s *vdec = hw_to_vdec(dec);
...
static void avs3_timeout_work_back(struct work_struct *work)
{
	struct AVS3Decoder_s *dec = container_of(work, struct AVS3Decoder_s, timeout_work_back);
	struct vdec_s *vdec = hw_to_vdec(dec);
...
static int avs3_recycle_frame_buffer(struct AVS3Decoder_s *dec)
{
	...
	for (i = 0; i < avs3_dec->max_pb_size; ++i) {
		if ((avs3_dec->pic_pool[i].is_ref == 0) &&
			(avs3_dec->pic_pool[i].buf_cfg.bg_flag == 0) &&
#ifdef NEW_FRONT_BACK_CODE
			(avs3_dec->pic_pool[i].buf_cfg.backend_ref == 0) &&
#endif
			(avs3_dec->pic_pool[i].buf_cfg.vf_ref) &&
			(avs3_dec->pic_pool[i].buf_cfg.index != -1) &&
			(avs3_dec->pic_pool[i].buf_cfg.is_display == 1) &&
			(avs3_dec->pic_pool[i].buf_cfg.cma_alloc_addr)) {
			index = avs3_dec->pic_pool[i].buf_cfg.index;
			aml_buf = (struct aml_buf *)dec->m_BUF[index].v4l_ref_buf_addr;
			avs3_print(dec,
				PRINT_FLAG_VDEC_DETAIL, "%s idx %d index %d vf_ref %d "
				"index %d cma_alloc_addr = 0x%lx vf_put_count %d poc %d\n",
				__func__, i, index,
				avs3_dec->pic_pool[i].buf_cfg.vf_ref,
				avs3_dec->pic_pool[i].buf_cfg.index,
				avs3_dec->pic_pool[i].buf_cfg.cma_alloc_addr,
				atomic_read(&dec->vf_put_count),
...
static unsigned long run_ready(struct vdec_s *vdec, unsigned long mask)
{
	...
	if (dec->next_again_flag &&
		...
		if (parser_wr_ptr >= dec->pre_parser_wr_ptr &&
			(parser_wr_ptr - dec->pre_parser_wr_ptr) < again_threshold) {
	...
	if (dec->pic_list_init_flag) {
		if (run_ready_max_vf_only_num > 0 &&
			get_vf_ref_only_buf_count(dec) >= run_ready_max_vf_only_num) {
			run_ready_case = 10;
			ret = 0;
		}
		if (run_ready_display_q_num > 0 &&
			kfifo_len(&dec->display_q) >= run_ready_display_q_num) {
			run_ready_case = 11;
			ret = 0;
		}
...
static void run_back(struct vdec_s *vdec, void (*callback)(struct vdec_s *, void *, int), void *arg)
{
	...
	if (vdec->mc_back_loaded || dec->front_back_mode != 1) {
		...
		*/
		...
		loadr = amhevc_vdec_loadmc_ex(VFORMAT_AVS3, vdec, "avs3_back", dec->fw_back->data);
	...
	void (*callback)(struct vdec_s *, void *, int), void *arg)
{
	...
	if (debug & PRINT_FLAG_VDEC_STATUS) {
		...
		if (vdec_frame_based(vdec) && dec->chunk) {
			u8 *data = NULL;
			if (!dec->chunk->block->is_mapped)
				data = codec_mm_vmap(dec->chunk->block->start + dec->chunk->offset, 8);
			else
				data = ((u8 *)dec->chunk->block->start_virt) + dec->chunk->offset;

			avs3_print_cont(dec, 0, "data adr %p:", data);
			for (ii = 0; ii < 8; ii++)
				avs3_print_cont(dec, 0, "%02x ", data[ii]);
			if (!dec->chunk->block->is_mapped)
				codec_mm_unmap_phyaddr(data);
		}
		avs3_print_cont(dec, 0, "\r\n");
	...
	if (vdec->mc_loaded) {
		...
		*/
		...
		int loadr = 0;
		if (dec->front_back_mode == 1 || dec->front_back_mode == 3)
			loadr = amhevc_vdec_loadmc_ex(VFORMAT_AVS3, vdec, "avs3_front", dec->fw->data);
		else
#endif
		if (amhevc_loadmc_ex(VFORMAT_AVS3, "avs3_mmu", dec->fw->data) < 0) {
			vdec->mc_loaded = 0;
			amhevc_disable();
			avs3_print(dec, 0, "%s: Error amvdec_loadmc avs3_mmu fail \n", __func__);
	...
	if (vdec_frame_based(vdec) && dec->chunk) {
		...
		r = dec->chunk->size + (dec->chunk->offset & (VDEC_FIFO_ALIGN - 1));
		if (vdec->mvfrm)
...
static void avs3_dump_state(struct vdec_s *vdec)
{
	...
	avs3_print(dec, 0, "====== %s\n", __func__);
	avs3_print(dec, 0, "width/height (%d/%d), max_pb_size %d\n",
	...
	avs3_print(dec, 0,
		"%s, newq(%d/%d), dispq(%d/%d), vf prepare/get/put (%d/%d/%d), free_buf_count %d get_free_buf_count %d (min %d for run_ready)\n",
		__func__,
	...
	if (input_frame_based(vdec) &&
		...
		if (dec->chunk && dec->chunk->block &&
			...
			u8 *data = NULL;
			if (!dec->chunk->block->is_mapped)
				data = codec_mm_vmap(dec->chunk->block->start + dec->chunk->offset, dec->chunk->size);
			else
				data = ((u8 *)dec->chunk->block->start_virt) + dec->chunk->offset;
			avs3_print(dec, 0, "frame data size 0x%x\n", dec->chunk->size);
...
static int ammvdec_avs3_probe(struct platform_device *pdev)
{
	...
	ctx->vdec_recycle_dec_resource = avs3_recycle_dec_resource;
	pdata->dec_status = vavs3_dec_status;
#ifdef I_ONLY_SUPPORT
