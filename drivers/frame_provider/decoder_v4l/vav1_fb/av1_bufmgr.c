...
static RefCntBuffer *assign_cur_frame_new_fb(AV1_COMMON *const cm) {
	...
	cm->cur_frame->buf.back_done_mark = 0;	//clear for new pic avoid pic_done output directly
#endif
#ifdef ORI_CODE
...
void av1_store_pbi_fb(AV1Decoder *pbi, AV1Decoder_fb *pbi_fb, u32 res_ch_flag)
{
	int i = 0;

	if (res_ch_flag) {
		pbi_fb->wait_working_buf = pbi->wait_working_buf;
		pbi_fb->fb_wr_pos = pbi->fb_wr_pos;
		pbi_fb->fb_rd_pos = pbi->fb_rd_pos;

		for (i = 0; i < MAX_FB_IFBUF_NUM; i++) {
			pbi_fb->next_bk[i] = pbi->next_bk[i];
			pbi_fb->next_be_decode_pic[i] = pbi->next_be_decode_pic[i];
		...
		pbi_fb->frontend_decoded_count = pbi->frontend_decoded_count;
		pbi_fb->backend_decoded_count = pbi->backend_decoded_count;
	}

	pbi_fb->HEVC_PARSER_PICTURE_SIZE_reg_val = pbi->HEVC_PARSER_HEADER_INFO_reg_val;
	pbi_fb->HEVC_PARSER_HEADER_INFO_reg_val = pbi->HEVC_PARSER_HEADER_INFO_reg_val;
	pbi_fb->HEVC_PARSER_HEADER_INFO2_reg_val = pbi->HEVC_PARSER_HEADER_INFO2_reg_val;

	pbi_fb->fb_buf_mmu0 = pbi->fb_buf_mmu0;
	pbi_fb->fb_buf_mmu1 = pbi->fb_buf_mmu1;
	pbi_fb->fb_buf_lcu_info_data0 = pbi->fb_buf_lcu_info_data0;
	pbi_fb->fb_buf_lcu_info_data1 = pbi->fb_buf_lcu_info_data1;
	pbi_fb->fb_buf_mpred_imp0 = pbi->fb_buf_mpred_imp0;
	pbi_fb->fb_buf_mpred_imp1 = pbi->fb_buf_mpred_imp1;
	pbi_fb->fb_buf_cu_info_data0 = pbi->fb_buf_cu_info_data0;
	pbi_fb->fb_buf_cu_info_data1 = pbi->fb_buf_cu_info_data1;
	pbi_fb->fb_buf_gmwm_data = pbi->fb_buf_gmwm_data;
	pbi_fb->fb_buf_lrf_data = pbi->fb_buf_lrf_data;
	pbi_fb->fb_buf_tldat_data0 = pbi->fb_buf_tldat_data0;
...
void av1_restore_pbi_fb(AV1Decoder *pbi, AV1Decoder_fb *pbi_fb, u32 res_ch_flag)
{
	int i = 0;
	if (res_ch_flag) {
		pbi->wait_working_buf = pbi_fb->wait_working_buf;
		...
		for (i = 0; i < MAX_FB_IFBUF_NUM; i++) {
			pbi->next_bk[i] = pbi_fb->next_bk[i];
			pbi->next_be_decode_pic[i] = pbi_fb->next_be_decode_pic[i];
			pbi->params[i] = pbi_fb->params[i];
		}

		pbi->frontend_decoded_count = pbi_fb->frontend_decoded_count;
		pbi->backend_decoded_count = pbi_fb->backend_decoded_count;
	}

	pbi->HEVC_PARSER_PICTURE_SIZE_reg_val = pbi_fb->HEVC_PARSER_HEADER_INFO_reg_val;
	pbi->HEVC_PARSER_HEADER_INFO_reg_val = pbi_fb->HEVC_PARSER_HEADER_INFO_reg_val;
	pbi->HEVC_PARSER_HEADER_INFO2_reg_val = pbi_fb->HEVC_PARSER_HEADER_INFO2_reg_val;

	pbi->fb_buf_mmu0 = pbi_fb->fb_buf_mmu0;
	pbi->fb_buf_mmu1 = pbi_fb->fb_buf_mmu1;

	pbi->fb_buf_lcu_info_data0 = pbi_fb->fb_buf_lcu_info_data0;
	pbi->fb_buf_lcu_info_data1 = pbi_fb->fb_buf_lcu_info_data1;
	pbi->fb_buf_mpred_imp0 = pbi_fb->fb_buf_mpred_imp0;
	pbi->fb_buf_mpred_imp1 = pbi_fb->fb_buf_mpred_imp1;
	pbi->fb_buf_cu_info_data0 = pbi_fb->fb_buf_cu_info_data0;
	pbi->fb_buf_cu_info_data1 = pbi_fb->fb_buf_cu_info_data1;

	pbi->fb_buf_gmwm_data = pbi_fb->fb_buf_gmwm_data;
	pbi->fb_buf_lrf_data = pbi_fb->fb_buf_lrf_data;
	pbi->fb_buf_tldat_data0 = pbi_fb->fb_buf_tldat_data0;
