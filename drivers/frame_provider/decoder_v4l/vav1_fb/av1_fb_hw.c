...
static void config_sao_hw_fb(struct AV1HW_s *hw, param_t* params)
{
	...
	av1_print(hw, AOM_DEBUG_HW_MORE,
		"####[config_sao_hw]#### lcu_size %d, lcu_total %d, dw_y_adr 0x%x, dw_uv_adr 0x%x, header_adr 0x%x, header_dw 0x%x\n",
		lcu_size, pic_config->lcu_total, pic_config->dw_y_adr, pic_config->dw_u_v_adr,
	...
	WRITE_BACK_32(hw, HEVC_SAO_Y_LENGTH, pic_config->luma_size);
	WRITE_BACK_32(hw, HEVC_SAO_C_LENGTH, pic_config->chroma_size);
...
#ifndef LOSLESS_COMPRESS_MODE
	/* multi tile to do... */
	WRITE_BACK_32(hw, HEVC_SAO_Y_WPTR , pic_config->mc_y_adr);
...
#else
		WRITE_BACK_32(hw, HEVC_DW_VH0_ADDDR, DOUBLE_WRITE_VH0_TEMP);
		WRITE_BACK_32(hw, HEVC_DW_VH1_ADDDR, DOUBLE_WRITE_VH1_TEMP);
#endif
...
void BackEnd_StartDecoding(struct AV1HW_s *hw)
{
	...
	if (hw->front_back_mode == 1) {
		if ((get_double_write_mode(hw) & 0x10) == 0) {
