...
typedef struct {
	uint32_t mmu0_ptr;
	uint32_t mmu1_ptr;
	uint32_t lcu_info_data0_ptr;
	uint32_t lcu_info_data1_ptr;
	uint32_t mpred_imp0_ptr;
	uint32_t mpred_imp1_ptr;

	uint32_t cu_info_data0_ptr;
	uint32_t cu_info_data1_ptr;
	uint32_t gmwm_data_ptr;
	uint32_t lrf_data_ptr;
	uint32_t tldat_data0_ptr;
...
typedef struct BuffInfo_s {
	...
	buff_t mpred_above;
	buff_t mpred_mv;
	buff_t fb_mpred_imp0;
	buff_t fb_mpred_imp1;
	buff_t fb_lcu_info_data0;
	buff_t fb_lcu_info_data1;
	buff_t fb_cu_info_data0;
	buff_t fb_cu_info_data1;
	buff_t fb_gmwm_data;
	buff_t fb_lrf_data;
	buff_t fb_tldat_data0;
	buff_t fb_tldat_data1;
	buff_t lmem;
	buff_t sequence_header_param;
	buff_t frame_header_param;
	buff_t tile_group_header_param;
	buff_t tile_header_param;
	buff_t fgs_ucode;
	buff_t vcpu_imem;
	buff_t system_imem;
	buff_t rpm;
...
#ifdef AML
//NEW_FRONT_BACK_CODE
	unsigned char inter_flag;
	int mmu_alloc_flag;
	int backend_ref;
	struct PIC_BUFFER_CONFIG_s *pic_refs[REF_FRAMES];
	struct scale_factors ref_scale_factors[REF_FRAMES];
	struct RefCntBuffer_s *ref_frame_map[REF_FRAMES];
	...
	uint32_t x0_qn_luma;
	uint32_t x0_qn_chroma;
	uint32_t x_step_qn_luma;
	uint32_t x_step_qn_chroma;
...
typedef struct AV1Decoder {
	...
	buff_t fb_buf_mmu0;
	buff_t fb_buf_mmu1;
	buff_t fb_buf_lcu_info_data0;
	buff_t fb_buf_lcu_info_data1;
	buff_t fb_buf_mpred_imp0;
	buff_t fb_buf_mpred_imp1;
	buff_t fb_buf_cu_info_data0;
	buff_t fb_buf_cu_info_data1;
	buff_t fb_buf_gmwm_data;
	buff_t fb_buf_lrf_data;
	buff_t fb_buf_tldat_data0;
	...
	PIC_BUFFER_CONFIG* next_be_decode_pic[MAX_FB_IFBUF_NUM];
